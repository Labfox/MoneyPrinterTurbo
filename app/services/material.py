...
def _get_search_provider(source: str):
    """
    Return a search callable for `source`, wrapped in the shared result cache.

    Going through one factory keeps every caller (plain term search and the
    interactive LLM search) on the same cached path, so repeated terms within a
    task do not hit the provider twice.
    """
    provider = "pexels"
