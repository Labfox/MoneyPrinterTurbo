...
def _notify_generation_complete(task_id, state):
    """
    在任务真正结束的那一次渲染里发送一次浏览器通知。

    快照渲染函数会在每次 Streamlit rerun 时重复执行，所以通知不能放在那里，
    否则用户每刷新一次就会收到一条重复通知。这里按 task_id 记录已通知状态。
    """
    if state != const.TASK_STATE_COMPLETE:
        return
    notified = st.session_state.setdefault("notified_generation_tasks", set())
...
def _render_running_generation_task(task_id):
    ...
    if state in {const.TASK_STATE_COMPLETE, const.TASK_STATE_FAILED}:
        ...
        _notify_generation_complete(task_id, state)
    ...
    with panel:
        with st.container(border=True):
            ...
            params.video_llm_search = st.checkbox(
                ...
                value=False,
                help=tr("LLM Interactive Search Help"),
            )
            _set_runtime_config(
                "app",
...
def _render_generation_controls(
    ...
    enable_system_notification_permission(tr("Generate Video"))
